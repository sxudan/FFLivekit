...
class AssetPlaybackManager: NSObject {
        ...
        didSet {
            ...
            playerItemObserver = playerItem?.observe(\AVPlayerItem.status, options: [.new, .initial]) { [weak self] (item, _) in
                ...
                if item.status == .readyToPlay {
                    ...
                    print("Error: \(String(describing: error?.localizedDescription)) \(error.debugDescription)")
                }
            }
        ...
        didSet {
            if let asset = asset {
                urlAssetObserver = asset.observe(\AVURLAsset.isPlayable, options: [.new, .initial]) { [weak self] (urlAsset, _) in
                    guard let strongSelf = self, urlAsset.isPlayable == true else {
                        print("Not playable")
                        return
                    }
     ...
     */
    func setAssetForPlayback(_ asset: AVURLAsset?) {
        self.asset = asset
    }
    ...
    func handlePlaybackStalled(_ notification: Notification) {
       perfMeasurements?.playbackStalled()
        self.delegate?.streamPlaybackManager(self, playerDidStalled: player)
    }
}
...
protocol AssetPlaybackDelegate: class {
    ...
    /// This is called when the internal AVPlayer is stalled
